...
#################################################
imb_technique = input("Type in the namne of the resampling technique among the followings -> Baseline / ADASYN / ALLKNN / CNN/ ENN/ IHT / NCR / NM / OSS / RENN / ROS / RUS / SMOTE / TOMEK: ")
...
print("ML algorithm: Deep neural network")
...
lr_f1_score_kfoldcv = [None] * (nsplits+2)
lr_ovr_accuracy_kfoldcv = [None] * (nsplits+2)

nb_f1_score_kfoldcv = [None] * (nsplits+2)
nb_ovr_accuracy_kfoldcv = [None] * (nsplits+2)

rf_f1_score_kfoldcv = [None] * (nsplits+2)
rf_ovr_accuracy_kfoldcv = [None] * (nsplits+2)

svm_f1_score_kfoldcv = [None] * (nsplits+2)
...
for train_index, test_index in kf.split(X_dummy):
    ...
    def get_recall(dnn_conf_matrix):
        dnn_tp_1 = dnn_conf_matrix[0][0]
        dnn_tp_2 = dnn_conf_matrix[1][1]
        dnn_tp_3 = dnn_conf_matrix[2][2]
        dnn_tp_4 = dnn_conf_matrix[3][3]
    ...
    dnn_recall = get_recall(dnn_conf_matrix)
    dnn_f1_score = 2 * (dnn_precision * dnn_recall) / (dnn_precision + dnn_recall)
    ...
    lr_recall = get_recall(lr_conf_matrix)
    lr_f1_score = 2 * (lr_precision * lr_recall) / (lr_precision + lr_recall)
    lr_ovr_accuracy = (lr_conf_matrix[0][0] + lr_conf_matrix[1][1] + lr_conf_matrix[2][2] + lr_conf_matrix[3][3] +
                       lr_conf_matrix[4][4]) / (
                              sum(lr_conf_matrix[0]) + sum(lr_conf_matrix[1]) + sum(lr_conf_matrix[2]) + sum(
                          lr_conf_matrix[3]) + sum(lr_conf_matrix[4]))
    print("lr_f1 score is:")
    print(lr_f1_score)
    ...
    nb_recall = get_recall(nb_conf_matrix)
    ...
    rf_recall = get_recall(rf_conf_matrix)
    ...
    def get_recall(svm_conf_matrix):
        svm_tp_1 = svm_conf_matrix[0][0]
        svm_tp_2 = svm_conf_matrix[1][1]
        svm_tp_3 = svm_conf_matrix[2][2]
        svm_tp_4 = svm_conf_matrix[3][3]
    ...
    svm_recall = get_recall(svm_conf_matrix)
